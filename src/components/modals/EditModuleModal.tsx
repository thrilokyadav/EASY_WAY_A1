...
interface EditModuleModalProps {
  ...
  module: Module | null;
  onClose: () => void;
  onEdit: (moduleId: number, moduleData: {
    prompt: string;
    en: ModuleContent;
    kn: ModuleContent;
  }) => void;
...
export default function EditModuleModal({ isOpen, module, onClose, onEdit }: EditModuleModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!module || !prompt.trim() || !enData.name.trim() || !knData.name.trim()) return;

    setIsSubmitting(true);
    try {
      await onEdit(module.id, { prompt, en: enData, kn: knData });
      onClose();
    } finally {
